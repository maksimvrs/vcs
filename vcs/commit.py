...
class Commit():
    ...
    def apply(self, tree=None, current_tree=None):
        ...
        for blob in current_tree.blobs:
            if tree is not None:
                blob_before = next((blob_before for blob_before in tree.blobs
                                    if blob_before.name == blob.name),
                                   None)
            ...
            # result.blobs.append(Blob(blob.name, blob.sha, blob.size,
            #                          changes=None, data=data))
        if current_tree.trees is not None:
            result.trees = list()
        for next_current_tree in current_tree.trees:
            if tree is not None:
                tree_before = next((
                    tree_before for tree_before in tree.trees
                    if tree_before.name == next_current_tree.name), None)
                result.trees.append(self.apply(
                    tree_before, next_current_tree))
            else:
